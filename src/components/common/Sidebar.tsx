...
// ── Active link bg tint ──────────────────────────────────────────────────────
const ACTIVE_BG = 'rgba(72,108,227,0.08)';
  ...
  return (
    <div className="mt-1">
      <button
        type="button"
        onClick={onToggle}
        className="w-full flex items-center justify-between px-3 py-1.5 rounded-lg transition-colors duration-150"
        style={{ color: hasActive ? 'var(--ink)' : 'var(--stone)' }}
            ...
            return (
              <Link
                ...
                style={{
                  ...
                  color: active ? 'var(--blue)' : 'var(--stone)',
...
const Sidebar = () => {
  ...
  useEffect(() => {
    setMounted(true);
    if (sessionStorage.getItem(STORAGE_KEY)  === 'true')  setMobileOpen(true);
    if (sessionStorage.getItem(WORKFLOW_KEY) === 'false') setWorkflowOpen(false);
    if (sessionStorage.getItem(SYSTEM_KEY)   === 'false') setSystemOpen(false);
  }, []);
  ...
  return (
    <>
      {/* ── Desktop Sidebar ── */}
      <aside
        className="pm-sidebar hidden md:flex"
        style={{ backgroundColor: 'var(--ice)', borderRight: '1px solid var(--cream)' }}
      >
        {/* Brand */}
        <div className="pm-sidebar-brand">
          <Link href="/pitches" className="focus:outline-none focus-visible:ring-2 rounded-lg" style={{ textDecoration: 'none' }}>
            <SidebarLogo />
          </Link>
        </div>
        ...
        <nav className="flex flex-col px-3 flex-1 overflow-y-auto" aria-label="Main navigation">
          ...
          <div className="my-2 border-t" style={{ borderColor: 'var(--cream)' }} />
      ...
      {/* ── Mobile Top Bar ── */}
      <header
        className="pm-topbar md:hidden"
        style={{ backgroundColor: 'var(--ice)', borderBottom: '1px solid var(--cream)' }}
      >
        <div className="flex items-center w-full px-4 gap-4">
          ...
          {activeNavItem && (
            <span style={{ fontFamily: "'Epilogue', sans-serif", fontSize: '0.875rem', color: 'var(--stone)' }}>
              {activeNavItem.label}
            </span>
          )}
          ...
          <button
            ...
            style={{ backgroundColor: 'transparent', border: '1px solid var(--cream)', color: 'var(--stone)' }}
      ...
      {drawerVisible && (
        <div className="md:hidden fixed inset-0 z-40" aria-hidden={!mobileOpen}>
          {/* Backdrop */}
          <div
            className={`absolute inset-0 transition-opacity duration-300 ${mobileOpen ? 'opacity-40' : 'opacity-0'}`}
