        ...
        {showSpotify && (
          <SpotifyArtistSearch
            ...
            onSelect={async (data: any) => {
              ...
              if (data.imageUrl) handleChange('avatar', data.imageUrl);
              if (data.genre) {
                const g = (data.genre || '').toLowerCase();
                const mapped =
                  g.includes('hip') || g.includes('rap')
                    ? 'Hip-Hop'
                    : g.includes('r&b') || (g.includes('soul') && g.includes('r'))
                      ? 'R&B'
                      : g.includes('soul')
                        ? 'Soul'
                        : g.includes('rock')
                          ...
                          : g.includes('metal')
                            ...
                            : g.includes('punk')
                              ? 'Punk'
                              : g.includes('indie') || g.includes('alternative')
                                ? 'Indie'
                                : g.includes('electro') ||
                                    g.includes('techno') ||
                                    g.includes('house')
                                            ...
                                            : g.includes('latin')
                                              ...
                                              : g.includes('folk')
                                                ...
                                                : g.includes('classical')
                                                  ? 'Classical'
                                                  : g.includes('gospel') || g.includes('christian')
                                                    ? 'Gospel'
                                                    : g.includes('k-pop') || g.includes('korean')
                                                      ? 'K-Pop'
                                                      : g.includes('afro')
                                                        ? 'Afrobeats'
                                                        : g.includes('ambient')
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-red-50 flex items-center justify-center">
              <Icon name="TrashIcon" size={18} variant="outline" className="text-red-500" />
            </div>
            <div>
              <h2 className="text-base font-semibold text-gray-900">Delete Artist</h2>
        ...
        <div className="px-6 py-5">
          <p className="text-sm text-gray-600">
            ...
            <span className="font-semibold text-gray-900">"{artistName}"</span>? All associated data
...
export default function ArtistsListingDashboard() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showToast(err.message || 'Failed to delete artist', 'error');
    }
  };
